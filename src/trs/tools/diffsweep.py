...
def _trsonly_test(rel, top, wk, testdir, src, trsonly):
    ...
    for line in open(trsonly, errors="replace"):
        ...
        if line.startswith("refuse="):
            ...
            # override the golden file (e.g. the design's Verilog-flow
            # golden when classic Bluesim cannot run it at all)
            golden_name = line[len("golden="):]
        elif line.startswith("filter="):
            # drop matching lines from BOTH sides before comparing:
            # documented sec 4.3 startup divergences (a 4-state
            # oracle's X-suppressed first task instant) and the
            # golden's own $finish banner
            filters.append(line[len("filter="):])
        elif line.startswith("normalize="):
            # normalize=ws collapses runs of spaces before comparing:
            # for goldens generated by a simulator with a different %d
            # field-width convention (sysParamOrder's predates the
            # iverilog-primary oracle contract; the VALUES must match)
            normalize = line[len("normalize="):]
    norm0 = (lambda t: re.sub(r" +", " ", t)) if normalize == "ws" \
        else (lambda t: t)

    def norm(t):
        t = norm0(t)
        for f in filters:
            t = "\n".join(l for l in t.split("\n") if not re.search(f, l))
        return t
    golden_p = os.path.join(testdir, golden_name)
    try:
    ...
    if norm(art.stdout) != norm(golden):
        return (rel, top, "DIFF",
                "artifact: " + diff_summary(norm(golden), norm(art.stdout)))
    ...
    if norm(inp.stdout) != norm(golden):
        return (rel, top, "DIFF",
                "run: " + diff_summary(norm(golden), norm(inp.stdout)))
