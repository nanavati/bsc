...
impl Interp {
    ...
    pub(crate) fn jit_plan(&mut self, rcomps: &[RComp]) -> Option<JitPlans> {
        ...
        // body-splitting reconnaissance (TRS_JIT_SPLIT_STATS=1):
        // per module type, how much cone mass is outlinable/memoizable
        if std::env::var_os("TRS_JIT_SPLIT_STATS").is_some() {
            ...
            for (&mir, &ex) in mirs {
                ...
                let thresh = std::env::var("TRS_JIT_SPLIT")
                    .ok()
                ...
                eprintln!(
                    "trs split: mir={mir} defs={ndefs} outlined={} stable={stable} piece-mass={mass} max-piece={maxeff}",
                    outlined.len()
                );
            }
        }
