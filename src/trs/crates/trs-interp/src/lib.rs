...
pub struct Interp {
    ...
    /// "<path>$<port>" -> (initial level, driver out-clock ordinal) for
    /// dynamic clocks whose edges are prim-triggered: the native clock
    /// prims (MakeClock, ClockDiv, ClockInverter -- always "$CLK_OUT",
    /// ordinal 0) and BVI output clocks (arbitrary port, per-contract
    /// ordinal)
    dynclk_init: HashMap<String, (bool, u32)>,
    /// (driver instance, out-clock ordinal) -> kernel clock index, for
    /// routing BVI output-clock edges discovered at the commit point;
    /// rebuilt in run() from the resolved clock sources
    bvi_clk_ci: HashMap<(usize, u32), usize>,
...
enum ClockSource {
    Wave(Wave),
    Triggered { init_high: bool, driver: usize, ord: u32 },
    Never,
}
...
impl Interp {
        ...
        for (name, kind, args) in child_specs {
            ...
            let cidx = match kind {
                ...
                ir::InstanceKind::Bvi(c) => {
                    ...
                    // output clocks: dynamic clock sources keyed
                    // "<path>$<port>" (the ClockDiv mechanism); edges
                    // are discovered at the commit point
                    for (ord, oc) in c.out_clocks.iter().enumerate() {
                        let pname = c.ports[oc.port as usize].name;
                        self.dynclk_init.insert(
                            format!("{cpath}${}", self.s(pname)),
                            (
                                clk_out_init.get(ord).copied().unwrap_or(false),
                                ord as u32,
                            ),
                        );
                    }
                    // output reset: joins the derived-reset network under
                    // the "<inst>$<port>" wire name (the native generator
                    // convention); its true initial condition is sampled
                    // at run() start after the t=0 cascades (bootstrap)
                    for orst in &c.out_resets {
                        let pname = c.ports[orst.port as usize].name;
                        let t1 = format!("{}${}", self.s(name), self.s(pname));
                        let t2 = format!("{}${}", self.s(name), self.s(orst.name));
                        let node = reset_map
                            .iter()
                ...
                ir::InstanceKind::Prim(p) => {
                    ...
                    match pname.as_str() {
                        "MakeClock" => {
                            ...
                            self.dynclk_init
                                .insert(format!("{cpath}$CLK_OUT"), (init_high, 0));
                        }
                        ...
                        | "UngatedClockMux" | "ClockSelect" | "UngatedClockSelect" => {
                            self.dynclk_init
                                .insert(format!("{cpath}$CLK_OUT"), (false, 0));
                        }
                        _ => {}
                    }
                    let idx = self.insts.len();
    ...
    /// Returns the derived output-clock edges the commit discovered, as
    /// (kernel clock index, new level); the caller heaps them at the
    /// current instant.
    fn bvi_commit_point(&mut self, now: u64) -> Vec<(usize, bool)> {
        let mut clk_edges: Vec<(usize, bool)> = Vec::new();
        ...
        for i in idxs {
            let (fin, edges) = match &mut self.insts[i].kind {
                InstKind::Prim(p) => (p.bvi_commit(now), p.take_clock_edges_multi()),
                _ => (false, Vec::new()),
            };
            finish |= fin;
            for (ord, lvl) in edges {
                if let Some(&ci) = self.bvi_clk_ci.get(&(i, ord)) {
                    clk_edges.push((ci, lvl));
                }
            }
        }
    ...
    fn resolve_clock_at(&self, inst: usize, wire: &str) -> ClockSource {
        ...
        if let Some(&(init_high, ord)) = self.dynclk_init.get(&abs) {
            let path = match abs.rfind('$') {
                Some(k) => &abs[..k],
                None => abs.as_str(),
            };
            let driver = *self
            ...
            return ClockSource::Triggered { init_high, driver, ord };
        }
    ...
    fn derive_plan_a(&mut self) -> PlanA {
        ...
        let driver_clock: HashMap<usize, usize> = sources
            ...
            .collect();
        // BVI output-clock routing: (driver instance, out-clock ordinal)
        // -> kernel clock index, read by the commit point
        self.bvi_clk_ci = sources
            .iter()
            .enumerate()
            .filter_map(|(ci, s)| match s {
                ClockSource::Triggered { driver, ord, .. } => {
                    Some(((*driver, *ord), ci))
                }
                _ => None,
            })
    ...
    pub fn prime(&mut self) {
        ...
        // BVI output-clock routing, rebuilt from the plan's sources
        self.bvi_clk_ci = sources
            .iter()
            .enumerate()
            .filter_map(|(ci, s)| match s {
        ...
        // clock-source prims alias the clock they DRIVE (native
        // "$CLK_OUT" keys and BVI output-clock "<path>$<port>" keys)
        for (ci, &c) in clocks.iter().enumerate() {
            let cname = self.s(c).to_string();
            if !self.dynclk_init.contains_key(&cname)
                && !cname.ends_with("$CLK_OUT")
            {
                continue;
            }
            if let Some(k) = cname.rfind('$') {
                if let Some(&pi) = self.inst_by_path.get(&cname[..k]) {
        ...
        for (eci, epos) in self.bvi_commit_point(0) {
            heap.push(Reverse((0, 1, eci, epos)));
        }
        // BVI output-reset bootstrap: the settled post-cascade level IS
        // the initial condition (not a transition) -- an inverter fed by
        // an initially-asserted reset starts DEASSERTED, so its
        // consumers run from slice 0 holding their uninitialized values
        // (the 4-state netlist fires the same way; its first display is
        // X-suppressed, a documented sec 4.3 startup divergence).  An
        // initially-ASSERTED output broadcasts pre-slice-0 like
        // InitialReset.  The bootstrap clears the transitions the t=0
        // cascade staged, so the flush below (native deferred asserts,
        // pre-slice-0 -- reset_init semantics) cannot double-apply them.
        for idx in self.bvi_insts.clone() {
            let Some(&node) = self.rstgen_out.get(&idx) else { continue };
            let asserted = match &mut self.insts[idx].kind {
                InstKind::Prim(p) => p.reset_out_bootstrap(),
                _ => None,
            };
            if asserted == Some(true) {
                self.apply_reset(node, true);
            }
        }
    ...
    pub fn advance_until(&mut self, cond: &StopCond) -> i32 {
        ...
        while self.fe.finished.is_none() && !self.fe.stop_request {
            ...
            if t > 2 && self.rst_asserted[0] {
                ...
                for (eci, epos) in self.bvi_commit_point(t) {
                    heap.push(Reverse((t, 1, eci, epos)));
                }
                self.flush_reset_pending();
                ...
                .filter(|(_, r)| r.clk == ci && r.posedge == pos)
            {
                ...
                if !ran_jit {
                    ...
                    // auto-fire anchors are positions in the BASE entry
                    // order: no front-end pairs -sched-dynamic with top
                    // auto-fire, and silently misplacing an anchor in a
                    // selected alternative would be a wrong byte
                    if sel.is_some() && !self.autofire.is_empty() {
                        panic!(
                            "trs: dynamic scheduling with auto-fired top \
            ...
            let mut same_time = matches!(heap.peek(), Some(Reverse((nt, _, _, _))) if *nt == t);
            if !same_time {
                // BVI batched edge commit: all of this instant's clock
                // levels and firing-caller inputs land in the models
                // (one NBA-batched eval per instance).  Derived output
                // clock edges heap at THIS instant and re-open the
                // slice: their subscribers (rules, prims, further BVIs)
                // run before the flush/PG_FINAL close it, and the
                // commit point re-runs after them (chained dividers
                // reach a fixpoint; instances with nothing new pending
                // early-return)
                for (eci, epos) in self.bvi_commit_point(t) {
                    heap.push(Reverse((t, 1, eci, epos)));
                }
                same_time =
                    matches!(heap.peek(), Some(Reverse((nt, _, _, _))) if *nt == t);
            }
            if !same_time {
                self.flush_reset_pending();
