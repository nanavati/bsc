...
answers one question the parity tests cannot: **how do bsim3's costs
...
- Every rule is provably always-fire (`fire_when_enabled,
  ...
  and its single output line is deterministic for the bsim3-vs-
  reference diff.
...
| `b3_link_s` | `bsim3 link <top>.bir -o b3sim` wall time |
| `ref_run_s`, `b3_run_s` | wall time of the two simulations |
| `ref_rss_kb`, `b3_rss_kb` | peak RSS (`/usr/bin/time -v`) |
| `ir_passes_s`, `backend_s` | `BSIM3_JIT_TIME=1` phase lines from the link ("ir passes" / "backend emit"; full log in `<work>/N<n>/b3link.log`) |
...
- `BSC`'s directory is put on `PATH` (the reference executable needs
  `bluetcl`); `BSIM3` is exported for the artifact wrapper script.
