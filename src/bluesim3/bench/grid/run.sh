...
# executable and the bsim3 AOT artifact from the same .bir, run both,
# diff stdout byte-for-byte, and append one CSV row of build/run/RSS
# numbers per N to results.csv.
#
#   BSC=/path/to/bsc BSIM3=/path/to/bsim3 [NS="2 4 8"] [CYCLES=1000] \
#       sh run.sh [workdir]
#
...
# the bsim3 artifact wrapper honors $BSIM3 (else wants bsim3 on PATH)
case "$BSIM3" in
    */*) BSIM3="$(cd "$(dirname "$BSIM3")" && pwd)/$(basename "$BSIM3")"
         PATH="$(dirname "$BSIM3"):$PATH"; export PATH;;
esac
export BSIM3
WK=${1:-$(mktemp -d)}
mkdir -p "$WK" || exit 2
...
# last BSIM3_JIT_TIME phase line containing $2, Duration rendered as
...
bench() { # n
    ...
    # bsim3 AOT link; BSIM3_JIT_TIME phase lines land in b3link.log
    t0=$(now)
    BSIM3_JIT_TIME=1 "$BSIM3" link "$top.bir" -o b3sim > b3link.log 2>&1 \
        || { echo "FAIL N=$n (bsim3 link, see $d/b3link.log)"; fail=1; return; }
    ...
    # phase lines: "bsim3 aot: ir passes 38.4ms", "bsim3 aot: backend
    ...
    [ "$rc_b3" = 0 ]  || { echo "FAIL N=$n (bsim3 exit $rc_b3)"; ok=0; }
