 ...
 * drive the bk_* lifecycle, and exercise the bsim3_* namespace —
 ...
 * expectation (bsim3_* has no reference implementation to mirror).
 *
 *   cc -o capi_witness capi_witness.c -ldl
 *   BSIM3_CAPI_ENGINES=interp,aot ./capi_witness b3_top.so top
 */
#include <dlfcn.h>
#include <stdio.h>
...
int main(int argc, char **argv) {
    ...
    unsigned (*engine_count)(void *) =
        (unsigned (*)(void *))dlsym(h, "bsim3_engine_count");
    const char *(*engine_kind)(void *, unsigned) =
        (const char *(*)(void *, unsigned))dlsym(h, "bsim3_engine_kind");
    unsigned char (*oracle_check)(void *) =
        (unsigned char (*)(void *))dlsym(h, "bsim3_oracle_check");
