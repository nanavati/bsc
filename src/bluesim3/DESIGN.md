# Bluesim 3: a Rust/LLVM simulation backend for BSC

Status: design proposal (working name "Bluesim 3"; user-facing name stays
...
- **bsim3 owns** everything downstream: the link closure, schedule merging,
...
format is a data contract, not an ABI: it is versioned, and `bsim3 ir dump`
pretty-prints it for diff-testing against bsc's own dump flags.

## 4. Execution semantics in bsim3
...
- FIFOs, BRAMs, RegFiles, synchronizers, clock/reset generators remain
  runtime primitives in Rust (`bsim3-rt`), *monomorphized by codegen*: the
...
- **Content-addressed object cache.**  Key = BIR hash ⊕ codegen options ⊕
  bsim3 version, mirroring PR #2's `StaleUtils` conventions ("missing product
  ...
  `mkFoo.o` via bsim3, and link reuses it — same mental model, same flags,
...
```
src/bluesim3/               Rust workspace (this directory)
  DESIGN.md                 this document
  crates/
    bsim3-ir/               BIR schema, loader, verifier, pretty-printer
    bsim3-kernel/           event queue, priorities, clocks, resets, bk_* core
    bsim3-rt/               primitives (FIFO, RegFile, BRAM, sync*), system
                            tasks, plusargs, wide-data helpers
    bsim3-wave/             change capture, VCD writer, FST writer
    bsim3-codegen/          LLVM lowering (feature "llvm", needs llvm-18-dev)
    bsim3/                  CLI: link planner, JIT/AOT driver, native runner
src/comp/SimExportIR.hs     (P0) BIR exporter, invoked from the -sim3 path
```

`cargo build` in `src/bluesim3` builds everything except `bsim3-codegen`
