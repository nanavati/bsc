...
fn main() -> ExitCode {
    ...
    match args.iter().map(String::as_str).collect::<Vec<_>>().as_slice() {
        ...
        ["link", path, rest @ ..] => {
            ...
            if interactive {
                ...
                match interp.aot_take_emit_result() {
                    Some(bsim3_interp::AotEmit::Compiled) => {}
                    Some(bsim3_interp::AotEmit::Failed(e)) => {
                        eprintln!("bsim3 link: {e}");
                        return ExitCode::FAILURE;
                    }
                    _ => eprintln!(
                        "bsim3 link: note: aot engine unavailable for \
        ...
        ["run", path, rest @ ..] => {
            ...
            let mut wave: Option<(bsim3_interp::WaveFormat, Option<String>)> =
                None;
            ...
            while let Some(a) = it.next() {
                match *a {
                    ...
                    p if p.starts_with('+') => {
                        ...
                        // rides after '='
                        if p == "+bscvcd" {
                            wave = Some((bsim3_interp::WaveFormat::Vcd, None));
                        } else if let Some(f) = p.strip_prefix("+bscvcd=") {
                            wave = Some((
                                bsim3_interp::WaveFormat::Vcd,
                                (!f.is_empty()).then(|| f.to_string()),
                            ));
                        } else if p == "+bscfst" {
                            wave = Some((bsim3_interp::WaveFormat::Fst, None));
                        } else if let Some(f) = p.strip_prefix("+bscfst=") {
                            wave = Some((
                                bsim3_interp::WaveFormat::Fst,
                                (!f.is_empty()).then(|| f.to_string()),
                            ));
                        }
...
fn run_script(
    ...
    plusargs: &[String],
    vcd: Option<&str>,
    wave: Option<(bsim3_interp::WaveFormat, Option<String>)>,
    code: Option<&str>,
    script: &str,
) -> ExitCode {
