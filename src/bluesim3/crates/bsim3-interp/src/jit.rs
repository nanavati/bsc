                ...
                match (self.kind)(mir, *instance) {
                    ...
                    ChildRef::User(cmir) => {
                        ...
                        match mm {
                            ...
                            Some(m) => {
                                if std::env::var_os("BSIM3_JIT_SPLIT_WHY").is_some() {
...
fn aot_emit(
    ...
    // BSIM3_AOT_ONE_MODULE=0 restores parallel chunked emission.
    let one_module = std::env::var("BSIM3_AOT_ONE_MODULE").as_deref() != Ok("0");
    if one_module {
        ...
        let comps: Vec<FusedComp> = comp_nodes
            ...
            .collect();
        let env = PlanEnv { d, insts: inst_envs, now_slot };
        let _g = bsim3_codegen::lower::AotModeGuard::set();
...
impl Interp {
    ...
    fn edge_ssa_plan(
        ...
        fn walk_expr(cx: &mut Ctx, inst: usize, e: &E, out: &mut Cone) {
            match e {
                ...
                E::Prim { op, args, .. } => {
                    ...
                    // poison hoistability.
                    if matches!(op, bsim3_ir::PrimOp::Quot | bsim3_ir::PrimOp::Rem)
                    {
                        out.poison |= 2;
                    }
                    for a in args {
        ...
        // BSIM3_EDGE_SSA_OUTLINE=<mass> forces an absolute threshold;
        // BSIM3_EDGE_SSA_OUTLINE_FACTOR tunes the model (0 disables).
        let outline_abs: Option<u64> = std::env::var("BSIM3_EDGE_SSA_OUTLINE")
            .ok()
            .and_then(|v| v.parse().ok());
        let outline_factor: u64 = std::env::var("BSIM3_EDGE_SSA_OUTLINE_FACTOR")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(2);
        ...
        for (k, comp_nodes) in nodes.iter().enumerate() {
            ...
            if stats && shared_defs > 0 {
                eprintln!(
                    "bsim3 edge-ssa: comp {k}: sections={} shared-defs={shared_defs} \
    ...
    pub(crate) fn jit_plan(&mut self, rcomps: &[RComp]) -> Option<JitPlans> {
        ...
        if let JitRequest::Emit { so } = &request {
            ...
            // BSIM3_EDGE_SSA=0 restores the classic emission
            let edge_plan = (std::env::var("BSIM3_EDGE_SSA").as_deref() != Ok("0"))
                .then(|| {
