...
pub trait Prim {
    /// Debug-tier symbol peeks (bsim3-capi): the prim's current value
...
thread_local! {
    /// debug (BSIM3_WARN_DEBUG): the trampoline token of the compiled
    ...
    fn value_method(&mut self, method: &str, args: &[Value], now: u64) -> Value {
        match method {
            "sub" => {
                let a = args[0].as_u64();
                if !self.in_range(a) {
                    if std::env::var_os("BSIM3_WARN_DEBUG").is_some() {
