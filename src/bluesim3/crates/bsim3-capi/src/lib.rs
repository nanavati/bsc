...
pub extern "C" fn bk_init(model: *mut c_void, _master: u8) -> *mut c_void {
    ...
    // companions travel beside the model .so (bsim3 link --interactive
        ...
        match Interp::from_bir_bytes(bir) {
            Ok(mut interp) => {
                    ...
                    && (bdpi_so.is_some() || interp.needs_user_bdpi())
                {
                    ...
                    eprintln!(
                        "bsim3 capi: note: multi-engine oracle with \
                ...
                if kind == EngineKind::Aot {
                    ...
                    match &aot_so {
                        ...
                        None => eprintln!(
                            "bsim3 capi: aot engine requested but no \
                             .aot.so beside the model (relink with a \
                             current bsim3); running interpreted"
                        ),
                    }
                }
                if !engines.is_empty() {
    ...
    if !hdl.is_null() {
        ...
        if let Some(r) = st.runner.take() {
            ...
            match r.join.join() {
                ...
                Err(_) => eprintln!(
                    "bsim3 capi: bk_shutdown: async worker panicked — \
    ...
    if is_async != 0 {
        ...
        let join = std::thread::spawn(move || {
            ...
            if caught_up {
                ...
                eprintln!(
                    "bsim3 oracle: catch-up interrupted — lockstep \
...
// =================================================================
// bsim3_* namespace (docs/TCL-CAPI.md): our capabilities beside the
// FROZEN bk_* surface — engine queries and oracle control.  The
// interactive link's export map whitelists both prefixes.

/// `bsim3_engine_count`: engines in this session; 0 while an async
/// run holds them (bk_sync first).
#[no_mangle]
pub extern "C" fn bsim3_engine_count(hdl: *mut c_void) -> u32 {
    state(hdl).engines.len() as u32
}

/// `bsim3_engine_kind`: engine i's kind as a static NUL-terminated
/// string ("interp" | "jit" | "aot"); NULL out of range or while an
/// async run holds the engines.
#[no_mangle]
pub extern "C" fn bsim3_engine_kind(hdl: *mut c_void, i: u32) -> *const c_char {
...
/// `bsim3_oracle_check`: run the lockstep + architectural-state
...
#[no_mangle]
pub extern "C" fn bsim3_oracle_check(hdl: *mut c_void) -> u8 {
    let st = state(hdl);
    if st.engines.is_empty() {
        return 2;
    }
...
/// "vcd" or "fst" — the bsim3 interp tier carries BOTH writers (the
...
pub extern "C" fn bk_set_waveform_format(
    ...
    let fmt = match name {
        "vcd" => bsim3_interp::WaveFormat::Vcd,
        "fst" => bsim3_interp::WaveFormat::Fst,
        other => {
            eprintln!(
...
pub extern "C" fn bk_get_waveform_format(hdl: *mut c_void) -> *const c_char {
    ...
    let fst = !st.engines.is_empty()
        && st.primary().wave_format() == bsim3_interp::WaveFormat::Fst;
    if fst {
