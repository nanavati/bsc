...
- 60e7b733 dlsym-complete surface: STOCK BLUETCL LOADS US.  Probe
  ...
  libbsim3_capi.a + stock export map) runs sim load/time/clock/
...
- docs/TCL-CAPI.md: the measured bluetcl contract (47 dlsym'd bk_*
  ...
  profile) + the bsim3-capi design for the DEBUG compile mode.
...
- 982afcd8 sysInit65536Bit AOT link timeout: LLVM known-bits is
  ...
  BSIM3_JIT_OPT still forces it.  Links 2.2s, byte-identical.
...
EDGE-SSA (#24) LANDED, opt-in BSIM3_EDGE_SSA=1 (d00f8954 emitter,
...
bsim3_edge_wire_ticks meta flag; interp skips covered entries only
...
bsim3 is FASTER THAN REFERENCE BLUESIM, byte-identical, from 0.48s
...
1. BDPI: VALUE IMPORTS DONE (d2d6e12e + bceb3f30) — direct C calls,
   ...
   BSIM3_JIT_PIPELINE / BSIM3_JIT_DUMP_PRE/POST / BSIM3_JIT_NOVEC.
...
REBASE PLAN (Ravi, 2026-07-09): rebase claude/bluesim3 onto upstream
...
-bir/-sim3/-c vs their -dump-formats), bsc.help.out.expected,
...
personal/bluesim-fst 534241d5 claude/bluesim3`, `git submodule
