...
module TCMisc(
        Bind, mkDefl,
        SolvedBinds, recursiveBinds, nonRecursiveBinds, sbsEmpty,
...
instance Semigroup SolvedBinds where
  new <> old
    ...
    where dependsOn s (_,_,e) = not $ S.disjoint (snd $ getFVE e) s
          ...
          (newlyRec, stillNonRec) = partition (dependsOn $ recursiveIds new) (nonRecursiveBinds old)
...
instance Monoid SolvedBinds where
  mempty = SolvedBinds {
             recursiveBinds = [],
             nonRecursiveBinds = [],
             recursiveIds = S.empty,
         ...
         m_equals -> do
          ...
          let fail msg =
                case m_equals of
                  ...
                  Nothing -> satTrace msg $ return ([p], mempty, nullSubst)
...
joinNeededCtxs = joinNeededCtxs' nullSubst mempty
...
reducePredsAggressive' dvs es sbs1 s1 vps1 = do
  (vps2, sbs2, s2) <- maskAllowIncoherent $ satMany dvs es [] mempty s1 vps1
  ...
  if (any badCon allPredTyCons) then do
    ...
    return (apSub s2 vps2, apSub s2 (sbs2 <> sbs1), s2)
...
defaultClasses fixedVars givenPreds unsatisfiedPreds =
   let
       ...
       tryDefault i ps Nothing t =
           do
              ...
              answer <- satMany (Just fixedVars) givenPreds [] mempty s ps'
   ...
   in
       if (S.null ambVarsSet) then
           -- no defaulting to do
           return (unsatisfiedPreds, mempty, [])
       else do
            let s0 = nullSubst
