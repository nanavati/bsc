...
sortRatForPrint :: [(MethodId, [(UniqueUse, Integer)])] -> [(MethodId, [(UniqueUse, Integer)])]
sortRatForPrint = sortBy cmp
    where cmp x y = compare (ppString x) (ppString y)

instance PPrint RAT where
  pPrint d p m = pPrint d p $ sortRatForPrint $ M.toList $ M.map M.toList m
...
rSchedule moduleId rFlag rMaxs rMap areSimult =
    let
        mapErr :: MethodId -> M.Map UniqueUse Integer -> M.Map UniqueUse Integer -> M.Map UniqueUse Integer
        mapErr m us1 us2 = internalError $ "Conflicting entries in resource allocation table for: " ++ ppReadable m ++ "\n" ++
                                           "entry 1: " ++ ppReadable us1 ++ "\nentry 2:" ++ ppReadable us2

        combineMaps = foldl (M.unionWithKey mapErr) M.empty
        concatTuple (xxs,yys) = (combineMaps xxs, concat yys)
