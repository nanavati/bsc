...
header :: [Byte]
header = B.unpack $ TE.encodeUtf8 $ T.pack "bsc-ba-20260707-1"
...
instance Bin VMItem where
    ...
    writeBytes (VMIfDef alts dflt)  = do putI 8; toBin alts; toBin dflt
    readBytes = do
      i <- getI
      case i of
        ...
        8 -> do alts <- fromBin; dflt <- fromBin; return (VMIfDef alts dflt)
