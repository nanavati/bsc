...
import TIMonad
import TCPat
import TCPatCheck(PatMatchContext(..), isGeneratedScrutinee)
import TCMisc
import CtxRed
import CSyntax
import CSyntaxUtil
...
tiExpr as td exp@(Ccase pos e arms) =
  do
    ...
    -- record the original arms for exhaustiveness/redundancy checking
    -- (unless the case was generated by the compiler, e.g. the degenerate
    -- unit-scrutinee encoding of BSV's plain "case" statement)
    when (not (isGeneratedScrutinee e)) $
        recordPatObligation PMCase pos clause_type clauses
...
tiExpl''' as0 i sc alts me (oqt@(oqs :=> ot), vts) = do
    ...
    recordPatObligation (PMDef i) (getPosition i) ot alts
...
tiImpl type_env type_var (i, (clauses, quals)) = do
        -- Typecheck the implicit condition (only for interfaces)
        ...
        recordPatObligation (PMDef i) (getPosition i) type_var clauses
