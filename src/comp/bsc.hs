...
import Data.List(intersect, nub, partition, intersperse, sort,
            isPrefixOf, isSuffixOf, unzip5, intercalate, tails)
...
import AOpt(aOpt)
import AVerilog(aVerilog, instGensClockOrReset, modUsesInoutPortExpressions)
...
vLink errh flags topmod_name vfilenames0 afilenames cfilenames = do
    ...
    -- Whether the design's generated Verilog keeps inout port
    -- expressions (shorted inouts) anywhere in its hierarchy (see
    -- modUsesInoutPortExpressions), which verilator cannot parse; the
    -- verilator build script is told via BSC_VSIM_INOUT_PORTS so it can
    -- refuse with a clear error.  Shorted boundary inouts are a
    -- topological property no conforming rendering avoids (Verilator
    -- rejects port aliases, 'alias' and 'tran' alike), so every module
    -- in the hierarchy is judged, like the timing analysis above.  The
    -- flags decide only whether InoutConnect instances were inlined in
    -- the module's .v: a .v regenerated by this link got this
    -- invocation's flags (see vGenMods), any other .v is assumed to
    -- match the flags stored in its .ba.  (In the hand-written-top
    -- fallback mod_abmis has only the .ba files from the command line,
    -- so inouts hiding in unlisted modules fail with verilator's own
    -- error instead of this one.)
    let stale_mod_names = [ abmiModName abmi | (_, abmi) <- stale_abmis ]
        renderFlags abmi | abmiModName abmi `elem` stale_mod_names = flags
                         | otherwise = abmi_flags abmi
        inout_ports = or [ modUsesInoutPortExpressions (renderFlags abmi)
                                                       (abmi_apkg abmi)
                         | (_, abmi) <- mod_abmis ]

    -- pass the info to vSimLink: array, location of files, -I, -L, -l
    start flags DFveriloglink
    vSimLink errh flags build_script topmod_name prefix vfilenames ofiles
        needs_timing inout_ports
...
vSimLink ::  ErrorHandle -> Flags -> String ->
             String -> String -> [VFileName] -> [String] -> Bool -> Bool ->
             IO ()
vSimLink errh flags build_script toplevel prefix vfiles ofiles
         needs_timing inout_ports = do
    let bsdir = bluespecDir flags
        ...
        -- whether the design's Verilog involves inout port expressions
        -- (which verilator cannot parse), analyzed from the .ba hierarchy
        inoutEnv = if inout_ports then "1" else "0"
        cmd = "BSC_VSIM_TRACE_FORMATS=" ++ dumpFmts ++ " " ++
              "BSC_VSIM_NEEDS_TIMING=" ++ timingEnv ++ " " ++
              "BSC_VSIM_INOUT_PORTS=" ++ inoutEnv ++ " " ++
