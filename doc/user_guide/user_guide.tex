...
When linking for Verilator, the design's \te{.ba} hierarchy is
required: BSC analyzes it to configure the build (for example, whether
the design needs a delay-based harness), so it is an error if the
\te{.ba} file of the top module or any submodule cannot be found.  For
other simulators, if no \te{.ba} files can be found for the design,
BSC issues a warning and links with the \te{.v} files as found.
...
\index{BSC\_VERILATOR\_TIMING@\texttt{BSC\_VERILATOR\_TIMING} (environment variable)}
When linking with \texttt{-vsim verilator}, BSC decides per design
whether to build the model with Verilator's \texttt{--timing} mode
(delay statements honored) or \texttt{--no-timing} (delays ignored;
the historical behavior): a design that generates a clock or reset
(for example via \texttt{mkAbsoluteClock}, clock dividers, or gated
clocks), or that is linked without BSC's \te{main.v}, depends on
delay-based library Verilog and is built with \texttt{--timing};
every other design is built with \texttt{--no-timing}.  The choice
can be overridden: \textbf{\texttt{BSC\_VERILATOR\_TIMING=1}} or
\texttt{-Xv --timing} forces timing mode, and
\texttt{BSC\_VERILATOR\_TIMING=0} or \texttt{-Xv --no-timing}
disables it (a design that needs timing is then refused with an
explanation).  Timing mode requires Verilator version 5 or later; in
this mode the \texttt{STMTDLY} and \texttt{INITIALDLY} lint waivers
are not applied, so Verilator reports delay-related warnings in
user-supplied Verilog rather than silently ignoring the delays.
