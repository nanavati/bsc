...
{\bf\tt T0159}) when the arms of a case expression, a pattern-matching
case statement, or the clauses of a function definition do not cover
all possible values of the matched expression, along with examples of
the values that are not matched.  When no pattern matches at
...
recognized as complete.  An arm
with a guard (\te{\&\&\&} or \te{when}) is conservatively assumed to
be able to fail and does not count towards completeness.  This flag
is on by default.

The {\bf\tt -warn-overlapping-patterns} flag displays a warning (tag
{\bf\tt T0160}) when a pattern in a case expression, a
pattern-matching case statement, or a function clause can never
match, because the preceding (unguarded) patterns already cover all
of the values that it matches.  This flag is on by default.
