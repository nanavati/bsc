...
A constant pattern is an integer literal, real literal, string literal,
or an enumeration label (such as \texttt{True} or \texttt{False}).
Binary, octal, and hexadecimal integer literals can include the wildcard
character \te{?} (example: \te{4'b00??}).
An integer or real literal may be preceded by a minus sign; as in
expressions, it matches the negated value (which, for the unsigned
types such as \te{Bit}, wraps around; for example, \te{-1} matches
the all-ones value).  A literal with wildcard characters cannot be
negated.
...
constant.  In a binary, octal, or hexadecimal integer pattern, each
wildcard character \te{?} independently matches any digit valid for
that base.  For example, \te{'h12?4} matches
\te{'h1204}, \te{'h1214}, through \te{'h12f4}.
